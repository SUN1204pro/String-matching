...
void BoyerMoore(const vector<vector<char>> &grid, const vector<string> &patterns, vector<string> &pos, long long &count)
{
    ...
    for (const string &word : patterns)
    {
        ...
        for (int i = 0; i < 256; i++)
            badchar[i] = -1;
        for (int i = 0; i < m; i++)
            badchar[(unsigned char)word[i]] = i;
        ...
        auto search = [&](const string &text, int line_idx, bool horiz)
        {
            ...
            while (s <= (n - m))
            {
                ...
                {
                    if (horiz)
                        matches.push_back(formatPos(line_idx, s, line_idx, s + m - 1));
                    else
                        matches.push_back(formatPos(s, line_idx, s + m - 1, line_idx));
        ...
        if (matches.empty())
            pos.push_back("not found");
        else
        {
            string res = "";
            for (size_t i = 0; i < matches.size(); i++)
                res += matches[i] + (i == matches.size() - 1 ? "" : "; ");
            pos.push_back(res);
        }
    }
}